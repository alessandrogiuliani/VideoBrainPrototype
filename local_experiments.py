...
def single_run(itemID):
    ...
    if args.gen_tags:
        tag_parameters  =  {'domain': args.domain,
                            ...
                            'get_original_tags': args.get_original_tags,
                            'top_trends': args.top_trends,
                            'rising_trends': args.rising_trends}
        tag_handler = TagGenerator(model=models[args.lang], **tag_parameters)
        suggested_tags_from_metainfo, suggested_trends, suggested_trends_from_title, channel_tag, title_tokens, yt_suggestions = tag_handler.getTags(itemID)
    ...
    {suggested_trends_from_title}\n\n- YouTube search bar suggestions: {yt_suggestions}\n\n'''
        local_folder = f'{os.getcwd()}/{args.output_folder_tags}/{itemID}'
...
def processCSV(csvfile):
        ...
        for v_id in toProcess:
            try:
                print(f'Processing video https://www.youtube.com/watch?v={v_id} - Video {i} of {length}')
                checkFolder = f'{os.getcwd()}/{args.output_folder_tags}/{v_id}'
                ...
                print(v_id + " cannot be processed.")
            i += 1
                
processCSV(args.file)