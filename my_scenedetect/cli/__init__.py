...
import my_scenedetect
from my_scenedetect.cli.context import CliContext
...
def print_help_header():
    ...
    click.echo(click.style(' PySceneDetect %s Help' % my_scenedetect.__version__, fg='yellow'))
    click.echo(click.style('----------------------------------------------------', fg='yellow'))
...
def scenedetect_cli(ctx, input, output, framerate, downscale, frame_skip, stats,
    ...
    if logfile is not None:
        ...
        logging.info('Version: %s', my_scenedetect.__version__)
...
def about_command(ctx):
    ...
    click.echo(click.style('----------------------------------------------------', fg='cyan'))
    click.echo(click.style(' About PySceneDetect %s' % my_scenedetect.__version__, fg='yellow'))
    click.echo(click.style('----------------------------------------------------', fg='cyan'))
    click.echo(my_scenedetect.ABOUT_STRING)
    ctx.exit()
...
def version_command(ctx):
    ...
    ctx.obj.process_input_flag = False
    click.echo(click.style('PySceneDetect %s' % my_scenedetect.__version__, fg='yellow'))
    ctx.exit()
...
def detect_content_command(ctx, threshold, min_scene_len):
    ...
    ctx.obj.add_detector(my_scenedetect.detectors.ContentDetector(
...
def detect_threshold_command(ctx, threshold, min_scene_len, fade_bias, add_last_scene,
    ...
    min_percent /= 100.0
    fade_bias /= 100.0
    ctx.obj.add_detector(my_scenedetect.detectors.ThresholdDetector(
